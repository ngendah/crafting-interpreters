...
value_t value_from_string_notable(const string_t str) {
...
value_t value_from_string(const string_t str, hash_table_t *strings_table) {
  return strings_table != nullptr ? value_from_string_table(str, strings_table)
                                  : value_from_string_notable(str);
}
...
bool value_compare(const value_t a, const value_t b) {
  if (a.type != b.type)
    return false;
  switch (a.type) {
  case VAL_BOOL:
  ...
  case VAL_STRING:
    return value_as_string(a).str == value_as_string(b).str;
  default:
    break;
  }
  return false;
